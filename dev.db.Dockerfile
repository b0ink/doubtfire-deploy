...
COPY .devcontainer/dev-mariadb.cnf /etc/mysql/mariadb.conf.d/

#  && \
#     sudo mkdir /student-work && \
#     sudo chown vscode:vscode /student-work

# TODO: setup_formatif_db.sh needs to be run on the main container but requires modification to check mysql remotely (to the cb container)

ENTRYPOINT [ "/workspace/.devcontainer/db-docker-entrypoint.sh" ]
CMD [ "sleep", "infinity" ]
